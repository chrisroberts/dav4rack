...
to without authentication. Note that Mongrel is much more responsive, so if you
are having slowness issues, install it and give that a try. The simple file resource
allows very basic authentication which is used for an example. To enable it:
